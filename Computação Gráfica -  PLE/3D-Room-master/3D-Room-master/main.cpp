...
void draw_bed() //cama
{
    ...
    glEnable(GL_TEXTURE_2D);
    glBindTexture(GL_TEXTURE_2D, texture[0]); // textura da cama
    glPushMatrix();
    glTranslatef(-4,0,-16);
    glScalef(12,3,6);
    draw_solid_cube(1);
    ...
    glEnable(GL_TEXTURE_2D);
    glBindTexture(GL_TEXTURE_2D, texture[0]); //cabeceira
    glPushMatrix();
...
void display(void)
{
    ...
    gluLookAt(camera_x, camera_height, camera_y, camera_x + cos(camera_x_angle),
     camera_height, camera_y + sin(camera_y_angle), 0, 1, 0);
...
int main(int argc, char **argv)
{
   ...
   // glClearColor(0,0,0,1);
    ...
    string path = (char*)"C:\\Workspace\\Computacao-Grafica--COMP269\\Computação Gráfica -  PLE\3D-Room-master\3D-Room-master\\"
    glGenTextures(10,texture);
    loadTexture(path + (char*)"bed_sheet.bmp", 0);  //Give full path of image else it will not work
    loadTexture(path + (char*)"wall.bmp", 1); // textura da parede
    loadTexture(path + (char*)"ceiling.bmp", 2); // textura teto
    loadTexture(path + (char*)"floor.bmp", 3); // textura piso
    loadTexture(path + (char*)"table.bmp", 4); //textura de madeira
    loadTexture(path + (char*)"shelf.bmp", 5); // outra madeira
